...
from telegram_bot.config import REGULAR_TTF, WIDTH_PROPORTION, \
    HEIGHT_PROPORTION, FILL_COLOR, STROKE_COLOR, GIF, DURATION, \
    STROKE_WIDTH, GIF_FILE_NAME, PNG_IMAGE, PNG
...
def create_gif(pictures: list[Image], watermark: str, user_id: int, private=False) -> BytesIO:
    """
    Creates gif with watermark from list of PIL images. Returns BytesIO object
...
def add_text(img, text):
    ...
    draw.text((set_up_text_location(draw, img, text, font)), text, fill=FILL_COLOR, stroke_fill=STROKE_COLOR,
...
def save_gif(images: list) -> BytesIO:
    ...
    images[0].save(
        gif, format=GIF, save_all=True, append_images=images[1:], duration=DURATION,
        loop=0
    )
    gif.seek(0)
    return gif
...
def set_up_text_location(image: ImageDraw,
                         img_opened: Image,
                         text: str,
                         font: ImageFont) -> Tuple[Any, Any]:
    """
    ...
    width, height = (img_opened.size[0] - width_text) * WIDTH_PROPORTION, \
                    (img_opened.size[1] - height_text) * HEIGHT_PROPORTION
    return width, height
